...
2. __Problem scenario__
    ...
    위에 나온 문제들을 해결하기 위해 논문에서는 인식 모델 $$q_{ϕ}(z\vert x)$$ 을 소개합니다. 이는 다루기 힘든 진짜 사후 $$p_{ϕ}(z\vert x)$$ 에 대한 근사치입니다. mean-field variational inference에서의 근사 사후와 달리, 이는 반드시 factorial되지 않으며, 파라미터 $$ϕ$$ 는 닫힌 형식의 기댓값으로부터 계산되지 않습니다. 대신에 논문에서는 인식 모델 파라미터 $$ϕ$$ 를 생성 모델 파라미터 $$\theta$$ 와 함께 학습하는 방법을 소개하고 있습니다.
...
3. __Algorithm__
    ...
    (5) gradients 계산 : 미니 배치 추정치에 대한 $$\theta$$ 와 $$\fi$$ 의 gradient를 계산합니다. $$L^{M}(\theta , ϕ ; X^{M} , ϵ)$$ 은 미니배치에 대한 목적함수(variational lower bound)입니다.
